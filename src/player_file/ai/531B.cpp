...
void PortOpen(int layer);//确保每一时刻只有一种单位在基地范围内
...
double parameter[4];
int LayerFree[3] = {1,1,1};//基地旁边1格该层无单位占据为1

int BaseMineCargo = -2, BaseFuelCargo = -2;  //三个补给基地的     初始化为-2，-1表示在生产
int base_defender[3] = {-2, -2, -2};      //留守基地的潜艇，初始化为 -2，-1表示在生产
int airrush[10];      //前u boat team，初始化为 -2，-1表示在生产，现飞机
int watcher[1] = {-2};//侦察机，初始化为 -2，-1表示在生产
...
Position CutOutPos[10];//放置cut out ecnomic的u boat的位置,现在为推基地
Position WatchingPos;//放置侦察机提供视野
...
//确保每一时刻只有一种单位在基地范围内layerfree
void PortOpen(int layer)
{
	int i;
	for(i=0;i<Info()->element_num;i++)
	{
		if(INFO->elements[i]->team==INFO->team_num	//待定是否注释掉
...
void Supply_Repair(int i)
{
	...
	else if(Element->type == BASE)
	{
		...
		if(Element->fuel < 0.4*kProperty[BASE].fuel_max && if_alive(BaseFuelCargo))
		{
			...
			INFO->elements[GetBase(INFO->team_num)]->metal > (Element->ammo - kProperty[Element->type].ammo_max))   //金属不足
...
void MoveCargo(int i)        
{
	...
	if(Element->type == CARGO && if_command(i, RETURNBASE)
		&& LayerFree[Element->pos.z]>=0)
	{
		if(Element->fuel < 0.4 * kProperty[CARGO].fuel_max||base->fuel<400)          //燃料不足
		{
			if(Element->index == BaseMineCargo)
			{
		...
		else if(Element->metal < 0.5 * kProperty[CARGO].metal_max||base->metal<100)     //金属不足
		{	
			ChangeDest(Element->index, INFO->elements[GetNear(Element->pos,MINE)]->pos);
			command[i][1] = CARGOSUPPLY;
			return;
...
void DefineUnit() //flag = 0,1,2,3对应BaseMineCargo, BaseFuelCargo, airrush, base_defender
 //i对应airrush的标号
{
