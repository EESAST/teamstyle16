...
int BaseMineCargo =-2, BaseFuelCargo[2] ={-2, -2};  //三个补给基地的     初始化为-2，-1表示在生产
...
void Supply_Repair(int i)
{
	...
	else if(Element->type == BASE)
	{
		...
		if(Element->fuel < 0.4*kProperty[BASE].fuel_max && if_alive(BaseFuelCargo[0]))
		{
			if(GetState(BaseFuelCargo[0])->fuel > 0.5*kProperty[CARGO].fuel_max)
				ChangeDest(BaseFuelCargo[0], Element->pos);
			else
			{
				int fuel = GetNear(Element->pos, OILFIELD);
				if(fuel <= 0)
					return;
				ChangeDest(BaseFuelCargo[0], INFO->elements[fuel]->pos);
			}
			for(int index=0; index<INFO->element_num; index++)
				if(INFO->elements[index]->index == BaseFuelCargo[0])
				{
					command[index][1] = RETURNBASE;
					return;
				}
			...
			|| (Element->fuel < distance(Element->pos, INFO->elements[GetBase(INFO->team_num)]->pos) + 9)))
	{
		...
		if(cargo_index >= 0  && cargo_index != BaseMineCargo && !if_in(cargo_index, BaseFuelCargo, 2))
		{
			...
			&& Element->type != CARGO)        //燃料不足
	{
		...
		//		return;
		//	}
		if(Element->type == CARRIER)
		{
			int cargo_index = GetNear(Element->pos, CARGO);
			if((INFO->elements[cargo_index]->ammo > kProperty[Element->type].ammo_max - Element->ammo
				|| INFO->elements[cargo_index]->fuel > 0.7 * kProperty[Element->type].fuel_max - Element->fuel)
				&& cargo_index != BaseMineCargo && !if_in(cargo_index, BaseFuelCargo, 2))
			{
				if(if_command(cargo_index, CARGOSUPPLY))
				{
					ChangeDest(INFO->elements[cargo_index]->index, Element->pos);
		...
		if((INFO->elements[index]->ammo > kProperty[Element->type].ammo_max - Element->ammo
				|| INFO->elements[index]->fuel > 0.7 * kProperty[Element->type].fuel_max - Element->fuel)
			&& index != BaseMineCargo && !if_in(index, BaseFuelCargo, 2))
		{
			if(if_command(index, CARGOSUPPLY))
			{
...
int GetNear(Position pos, ElementType type)
{
	...
	for(i=0;i<INFO->element_num;i++)
	{
		...
		{
			if( type == OILFIELD /*&& 
				distance(target->pos, INFO->elements[GetBase(1 - INFO->team_num)]->pos) > kProperty[BASE].fire_ranges[SURFACE]*/
				&& (target->fuel > 40 || !target->visible)) //油田还有石油
			{
				near_index = i;
				...
				distance(target->pos, INFO->elements[GetBase(1 - INFO->team_num)]->pos) > kProperty[BASE].fire_ranges[SURFACE]*/
				&& (target->metal > 20|| !target->visible))//保证矿场还有金属
			{
				near_index = i;
				min_distance = distance(target->pos, pos);
			}
	...
	for(int j=0;j<INFO->element_num;j++)
	{
			...
			&& target->team == INFO->team_num)
		{
			...
			//{
				...
				int fuel_num = (0.7*element->fuel) < (element->fuel-50)? (0.7*element->fuel):(element->fuel-50);
				Supply(INFO->elements[index]->index, target->index, fuel_num, 0, element->metal);
			//}
				...
				/*if(if_in(element->index, BaseFuelCargo, 2))
				{
					int fuel = GetNear(element->pos,OILFIELD);
					if(fuel < 0)
						return;
			...
			&& target->type != FORT)
		{
			...
			{*/
				...
				int fuel_num = (kProperty[target->type].fuel_max - target->fuel) < (element->fuel-50)? (kProperty[target->type].fuel_max - target->fuel):(element->fuel-50);
				Supply(INFO->elements[index]->index, INFO->elements[j]->index, fuel_num,
...
void MoveCargo(int i)        
{
	...
	if(Element->type == CARGO && if_command(i, RETURNBASE))
	{
		...
		else if((Element->index == BaseMineCargo || if_in(Element->index, BaseFuelCargo, 2))
	...
	if(BaseMineCargo <= 0 || !if_alive(BaseMineCargo))
	{
		...
		for(int j=0; j<INFO->element_num; j++)
		{
			...
			if(Element->team == INFO->team_num && Element->type == CARGO 
				&& Distance > distance(Element->pos, base->pos) && Element->index != BaseFuelCargo[j])
			{
				Distance = distance(Element->pos, base->pos);
				index = INFO->elements[j]->index;
			}
...
void ScoutGo(int index)
{
	const State *Element = INFO->elements[index];
	if(if_command(index, FORWARD))
	{
		Position dest;
		Position Base = INFO->elements[GetBase(INFO->team_num)]->pos;
		dest.x = INFO->x_max/2;
		dest.y = (2 + ((Base.y < INFO->y_max/2)?1:-1))*(INFO->y_max/4);
		ChangeDest(Element->index, dest);
	}
}
