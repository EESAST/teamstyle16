...
//主函数中调用各项功能函数，for循环后对i判断各个单位的索引号，分类进行各项活动
void AIMain()
{  
	TryUpdate();
	//回合开始时的初始化
...
void init()
{
	if(BEGINGAME == 0)
	{
		...
		BaseDefendPos.y = Base->pos.y+5*(Base->pos.y < INFO->y_max/2?1:-1);     //坐标也可以直接加减等
		BaseDefendPos.z = 1;//确定驱逐舰的位置
...
void enemy_init()
{
	...
	for(int i=0;i<Info()->element_num;i++)
		...
		{
			enemy_num += 1;
			Enemy_Indexes.push_back(i);//寻找敌方可见单位（index）
		}	
	enemy_element = new State[enemy_num];
	for(int i=0; i<enemy_num; i++)
		{
			enemy_element[i] = *(Info()->elements[Enemy_Indexes[i]]);//形成一个新的敌方单位指针数组
		}
	Enemy_Indexes.clear();
}
...
//flag = 0,1,2对应BaseMineCargo, BaseFuelCargo, base_defender
 //i对应base_defender的标号
void DefineUnit() 
{
