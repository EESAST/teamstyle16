...
inline bool InFireRange(const State& a, const State& b)		{
	...
	if(a.type!=BASE &&b.type!=BASE){return InFireRange(a,b.pos);}
	else if(b.type!=BASE) {return DistanceToBase(a.pos,b.pos)<=kProperty[BASE].fire_ranges[b.pos.z];}
	else if(a.type!=BASE) {return DistanceToBase(b.pos,a.pos)<=kProperty[a.type].fire_ranges[b.pos.z];}
	else {return Distance(a.pos,b.pos)-4<=kProperty[a.type].fire_ranges[b.pos.z];}
}
