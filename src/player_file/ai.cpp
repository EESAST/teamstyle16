...
{  
	...
	for(int i=0;i<Info()->element_num;i++)
		if(Info()->elements[i].team == Info()->team_num)
			FormerElement[i] = Info()->elements[i].index;
...
void init()
{
	...
	enemy_num =0;
	for(int i=0;i<Info()->element_num;i++)
		if(Info()->elements[i].team != Info()->team_num  && Info()->elements[i].type != MINE && Info()->elements[i].type != OILFIELD)
			enemy_num += 1;
	...
	for(int i=0,j=0;i<Info()->element_num;i++)
		...
		{
			enemy_element[j] = Info()->elements[i];
			j++;
		}
}
...
void Cargo_Supply(int index)                           
{
	for(int j=0;j<INFO->element_num;j++)
		if(distance(INFO->elements[index].pos,INFO->elements[j].pos) <= 1 && index!=j && INFO->elements[j].team == INFO->team_num)
		{
...
void Forward(int index)
{
	...
	else if(Element.type == CARGO && ! if_in(Element.index, cargo_index, 2))
	{
		for(int i=0;i<INFO->element_num;i++)
			...
			{
				Distance = distance(Element.pos,INFO->elements[i].pos) < Distance ? distance(Element.pos,INFO->elements[i].pos) : Distance;
