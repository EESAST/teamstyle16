...
* 只能在同一层次内移动，不能穿越同层其他单位  @逻辑：请确认不能穿越其他单位
* 每移动一格消耗一单位燃料（@逻辑：每次移动究竟是走直线还是斜线？），燃料<=0则不能继续移动。
  特别地，若飞机编队燃料降至零，则坠毁（相当于自爆操作），并对当前坐标的1层（水面/地面）单位造成一定伤害。
...
* 对空维修距离为0，其余维修距离为1
* 一次只能维修一个单位，维修补满生命值，弹药及燃料
* 若被维修单位为飞机，则类似于生产操作，可选择维修后的编队配置方案  。@逻辑：消耗资源为多少？被替换飞机直接消失？

#### 生产
基地特有指令，给出编码后的单位类型号，生产该类型单位

* 生产后自动附加补给操作
* 生产出的单位在到基地距离为1的对应层次随机出生  @逻辑：无处可放怎么办？
...
### 结算顺序
结算分为 **战斗、资源、生产、移动** 四个阶段，每阶段内按下表所示顺序结算
